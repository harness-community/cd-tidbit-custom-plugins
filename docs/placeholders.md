...
| `KANBOARD_PROJECT_ID`, `KANBOARD_TASK_ID`, `KANBOARD_COL_*` | Written by `setup.sh` (JSON-RPC bootstrap) → consumed by pipeline / env variables | Identify which board, which task, and which column maps to which env. `setup.sh` fills these in on first run |
