...
|---|---|---|
| Plugin step (containerized) | `.harness/pipeline.yaml` — Plugin step inside each env stage | The pipeline YAML shows a `type: Plugin` step pointing at a Docker image; the execution view shows three Plugin step instances per run |
